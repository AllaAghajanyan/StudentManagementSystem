...
def data_input():
    ...
    current_year_grade = Validations.valid_float("Enter student's current year grade: ")
    last_year_grade = Validations.valid_float("Enter student's last year grade: ")
    return name, surname, age, current_year_grade, last_year_grade
        ...
        with open(file_path, "r") as file:
            for line in file:
                ...
                try:
                    ...
                    current_year_grade = float(student_info[3])
                    last_year_grade = float(student_info[4])
                except ValueError:
                    print(f"Invalid numeric data. Skipping line: {line.strip()}")
                    continue

                students_data.append((name, surname, age, current_year_grade, last_year_grade))
